...
export function OrganisationDetailClient({ organisation }) {
  const { toast } = useToast()
  const orgId = organisation.id

  // Branding state
  const [logoUrl, setLogoUrl] = useState(organisation.brand_settings?.logo_url || null)
  const [heroImageUrl, setHeroImageUrl] = useState(organisation.brand_settings?.hero_image_url || null)
  const [primaryColor, setPrimaryColor] = useState(organisation.brand_settings?.primary_color || "#001F3F")
  const [secondaryColor, setSecondaryColor] = useState(organisation.brand_settings?.secondary_color || "#d4af37")
